...
struct capability_info exitbase[14] =
{
...
struct capability_info entrybase[12] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Secondary Procbased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(secprocbased, 27, lo, hi);

	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("VM-Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exitbase, 14, lo, hi);

	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("VM-Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entrybase, 12, lo, hi);
}
